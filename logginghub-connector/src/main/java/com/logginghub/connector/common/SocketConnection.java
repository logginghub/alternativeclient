...
/**
 * Connection handler for socket based logging messaging connections. As both reading and writing
 * are potentially blocking operations, each connection maintains two worker threads to manage IO.
 * 
 * @author admin
 */
public class SocketConnection implements LoggingMessageSource, FilteredMessageSender, Destination<LogEvent>, QueueAwareLoggingMessageSender {
    ...
    /**
     * Each sweep we use this list to store the messages to be sent - I'm not sure if its going to
     * be quicker to create a new empty list than it is to clear it down
     */
     ...
     * @param socket
     * @param listener
     *            In order to ensure you dont miss any messages, it is important to add a listener
    ...
    public void sendInternal(LoggingMessage message) throws IOException {
        if (logger.willLog(Logger.finer)) {
            logger.fine("Sending message '{}', sendBuffer is currently '{}'", message, sendBuffer);
        }

        codex.encode(sendBuffer, message);

        if (logger.willLog(Logger.finer)) {
            logger.fine("Codex encoded to sendBuffer '{}'", sendBuffer);
        }
    ...
    public void sendBufferInternal(ExpandingByteBuffer buffer, LoggingMessage message) throws IOException {
        ...
        logger.fine("Send buffer is now '{}'", buffer);
    }
    ...
    private synchronized void close(String reason, Object... params) {
        boolean needToAct;
    ...
    public void send(LogEvent logEvent) {
        ...
        if (logEvent.getLevel() >= getLevelFilter()) {
            ...
            logger.fine("Connection {} has a level filter that blocks this event ({} < {})", this, getLevelFilter(), logEvent.getLevel());
        }
    }
