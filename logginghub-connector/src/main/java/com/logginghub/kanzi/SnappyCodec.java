...
public final class SnappyCodec implements ByteFunction {
    ...
    public boolean forward(IndexedByteArray source, IndexedByteArray destination) {
        ...
        if (dst.length - destination.index < getMaxEncodedLength(count)) return false;
        ...
        while (srcIdx < ends2) {
            ...
            // If t is invalid or src[srcIdx:srcIdx+4] differs from src[t:t+4], accumulate a literal
            // byte
    ...
    private static long getUvarint(IndexedByteArray iba) {
        ...
        for (int i = idx; i < len; i++) {
            ...
            if (((s == 63) && (b > 1)) || (s > 63)) throw new NumberFormatException("Overflow: value is larger than 64 bits");
    ...
    public boolean inverse(IndexedByteArray source, IndexedByteArray destination) {
        ...
        if ((dLen < 0) || (dst.length - dstIdx < dLen)) return false;
            ...
            while (s < ends) {
                switch (src[s] & 0x03) {
                    case TAG_LITERAL: {
                        ...
                        if ((length <= 0) || (length > dst.length - d) || (length > ends - s)) return false;
                ...
                if ((offset > d) || (end > dst.length)) return false;
    ...
    private static int readInt(byte[] array, int srcIdx) {
        return ((array[srcIdx] & 0xFF)) | ((array[srcIdx + 1] & 0xFF) << 8) | ((array[srcIdx + 2] & 0xFF) << 16) | ((array[srcIdx + 3]) << 24);
    }
