...
public final class LZ4Codec implements ByteFunction {
    ...
    public LZ4Codec(int size) {
        if (size < 0) throw new IllegalArgumentException("Invalid size parameter (must be at least 0)");
    ...
    private static int emitLiterals(IndexedByteArray source, IndexedByteArray destination, int runLen, boolean last) {
        int token;
        ...
        if (runLen >= RUN_MASK) {
            ...
            if (last == true) destination.array[destination.index++] = (byte) token;
        ...
        // Emit literals
        if (last == true) System.arraycopy(source.array, source.index, destination.array, destination.index, runLen);
        else customArrayCopy(source.array, source.index, destination.array, destination.index, runLen);
    ...
    public boolean forward(IndexedByteArray source, IndexedByteArray destination) {
        if ((source == null) || (destination == null) || (source.array == destination.array)) return false;
        ...
        if (destination.array.length - destination.index < getMaxEncodedLength(count)) return false;
        ...
        while (true) {
            ...
            do {
                ...
                if ((differentInts(src, ref, srcIdx) == true) || (ref <= srcIdx - MAX_DISTANCE)) break;
    ...
    private static boolean differentInts(byte[] array, int srcIdx, int dstIdx) {
        return ((array[srcIdx] != array[dstIdx]) || (array[srcIdx + 1] != array[dstIdx + 1]) || (array[srcIdx + 2] != array[dstIdx + 2]) || (array[srcIdx + 3] != array[dstIdx + 3]));
    }
